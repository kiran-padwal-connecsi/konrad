$(document).ready(function () {
                ...
                if (data.results.length != 0){
                    jQuery.each(data.results, function(i, val) {
                        ...
                        if(val.twitter_channel_id){
                            ...
                            $var = '<div class="custom-control custom-control-inline-block custom-checkbox custom-control-inline text-left"><input type="checkbox" class="filled-in custom-control-input custom-control-input" multiple="" value="twitter@'+val.twitter_channel_id+'" name="proposal_channels" id="ch_twitter_id_'+$uniqid+'"><label class="custom-control-label campaignCheckbox" for="ch_twitter_id_'+$uniqid+'"><span data-toggle="tooltip" title="'+val.twitter_first_name+'"><a target="_blank" href="#" class="fa fa-twitter"></a></span></label></div>';
                            $("#proposal_channels").append($var);
                        }
            ...
            success: function(data)
            {
                jQuery.each(data.results, function(i, val) {
                    ...
                    for(i=0;i<arrangements.length;i++){
                        $uniqid = $uniqid + 1;
                        $var = '<div class="custom-control custom-control-inline-block custom-checkbox custom-control-inline text-left"><input type="checkbox" class="filled-in custom-control-input custom-control-input" multiple="" value="'+arrangements[i]+'" name="proposal_arrangements" id="arrangements_id_'+$uniqid+'"><label class="custom-control-label campaignCheckbox" for="arrangements_id_'+$uniqid+'">'+arrangements[i]+'</label></div>';
                        $("#proposal_arrangements").append($var);
                    }
                    var kpis = val.kpis.split(",");
                    for(i=0;i<kpis.length;i++){
                        $uniqid = $uniqid + 1;
                        $var = '<div class="custom-control custom-control-inline-block custom-checkbox custom-control-inline text-left"><input type="checkbox" class="filled-in custom-control-input custom-control-input" multiple="" value="'+kpis[i]+'" name="proposal_kpis" id="kpis_id_'+$uniqid+'"><label class="custom-control-label campaignCheckbox" for="kpis_id_'+$uniqid+'">'+kpis[i]+'</label></div>';
    ...
    $('#message_agreements_form').on('change', 'input:file',function(){
        ...
        // $(".displayMessageAgreements").append('<input type = "file" name="message_agreements">');
        // $('#message_agreements_form').submit();
    });
